...
// Keep this aligned with the `scope:`/`REQUIRED_SCOPES` strings used across the
// universal_components SDK use cases and view models (grep for `:me:` in
// universal_components/src/main) — the client grant and the native client's
// refresh-token policy are provisioned with precisely these, so an unused scope
// here would over-request access the app never exercises.
//   read:me:authentication_methods    — list enrolled authenticators
//   create:me:authentication_methods  — enroll passkey/TOTP/email/SMS/recovery
